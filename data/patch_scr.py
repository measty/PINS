...
for slide_numder in slides:
    ...
    for img_path in images:
        ...
        with Image.open(img_path) as img:
            ...
            extractor=SlidingWindowPatchExtractor(np_img,(psize,psize),'otsu', pad_constant_values=255)
            coords=extractor.coord_list
            patches, patch_paths,stats=[],[],[]
            for i, patch in enumerate(extractor):
                    coord=coords[i,:]
                    patch = Image.fromarray(patch)
                    blur_patch=np.array(patch.copy().filter(ImageFilter.GaussianBlur(radius=4)))
                    ...
                    if norm:
                        try:
                            normed_patch = stain_normalizer.transform(np.asarray(patch).copy())
                        except:
                    ...
                    patch_paths.append(save_path.joinpath(f'{img_path.stem}_{coord[0]}_{coord[1]}_{label}.tiff'))
