        ...
        for k in range(int(batches_per_slide)):
            ...
            for i in perm:
                rand_tensor = torch.multinomial(
                    self.weights[csum[i] : csum[i + 1]],
                    self.btch_size,
                    self.replacement,
                    generator=self.generator,
                )
                rand_list.extend(
                    (rand_tensor + csum[i]).tolist()
                )  # do i really want to sample same number per slide?
    ...
    def __init__(
        ...
        if num_samples < 0:
            ...
            if same_per_slide:
                ...
                self.num_samples = (
                    self.samples_per_slide * self.nslides
                )  # samples per slide
            else:
                self.samples_per_slide = np.floor(
                    sizes * num_samples / sum(sizes)
                ).astype("uint64")
