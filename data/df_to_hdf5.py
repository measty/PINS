    ...
    with h5py.File(vds_path, 'w', libver='latest') as f:
        for i, fname in enumerate(fnames):
            with h5py.File(data_path.joinpath(fname+'.hdf5'), 'r') as f2:
                ...
                npatches=len(range(offset,sh[0]-rough_step+offset+1,rough_step))
                ...
                h_channel.extend(f2['h_channel'][offsets[i]:orig_sizes[i]-step_sizes[i]+offsets[i]+1:step_sizes[i]])
                slide_labels.append(to_binary_class(df_labels.iloc[i]))
        
        
        layout = h5py.VirtualLayout(shape=(sum(sizes),3,224,224), dtype=h5py.h5t.STD_U8BE)
        ind=0
        for i in range(len(fnames)):
            layout[ind:ind+sizes[i],:,:,:]=vsources[i][offsets[i]:orig_sizes[i]-step_sizes[i]+offsets[i]+1:step_sizes[i],:,:,:]
            ind+=sizes[i]
