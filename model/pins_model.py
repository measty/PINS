...
import pickle
import model.av_agg_pred_metric as agg
...
info_str = f"pre_stage={pre_stage}, MIL_type={MIL_type}, batch_size={batch_size}, nsample={nsample}, agg_topN={agg_topN}, epochs={epochs}, sample_factor={sample_factor}, num_workers={num_workers}, lr={lr}, wd={wd}, same_per_slide={same_per_slide}, extra_info={extra_info}, samp_alpha={samp_alpha}, optim={optim}"
result_str = []
...
def get_data_loaders(split_df=None, base_path=None, fracs=[0.6, 0.2, 0.2]):
    ...
    trans_tr = transforms.Compose(
        [
            ...
            transforms.ColorJitter(
                brightness=0.1, contrast=0.05, saturation=0.2, hue=0.2
            ),
    ...
    trans_tr_val = transforms.Compose([im_trans(), torch.from_numpy, norm])
    trans_val = transforms.Compose([im_trans(), torch.from_numpy, norm])
    tar_trans = lab_trans()
    ...
    for key in dfs.keys():
        ...
        if key == "Train":
            ...
            h_weights = ds.get_weights(alpha=samp_alpha)
            NS = int(np.maximum(sample_factor, sample_factor * len(ds)))
            if pre_stage == "sarc":
                labs = ds.get_labels()
                argh = sum(h_weights[labs == 0]) / (
                    sum(h_weights[labs == 1]) * class_ratio
                )
                h_weights[labs == 1] = argh * h_weights[labs == 1]
                sampler = WeightedRandomStratifiedSampler(
                    h_weights,
                    int(0.2 * NS),
                    sizes=ds.sizes,
                    same_per_slide=same_per_slide,
                )
            elif MIL_type != "batch" or pre_stage == "normal":
                sampler = WeightedRandomStratifiedSampler(
                    h_weights, NS, sizes=ds.sizes, same_per_slide=same_per_slide
                )
            else:
                sampler = WeightedMILSampler(
                    h_weights, 70, sizes=ds.sizes, btch_size=batch_size
                )
            ...
            data_loaders["Tr_eval"] = DataLoader(
                ds,
                batch_size=batch_size,
                shuffle=False,
                num_workers=num_workers,
    ...
    for i, size in enumerate(sizes):
        if i == 0:
            torch.cat(
                (
                    y_pred_out,
                    torch.unsqueeze(torch.mean(y_pred[0 : csum_sizes[i], :], 0), 0),
                ),
                0,
            )
        else:
            y_pred_out.append(
                torch.mean(y_pred[csum_sizes[i - 1] : csum_sizes[i], :], 0)
            )
...
def log_pr_image(preds, labels, ap, logger, tag, global_step):
    ...
    display = PrecisionRecallDisplay(
        precision=prec, recall=recall, average_precision=ap
    )
    display.plot()
    logger.writer.add_figure(tag, display.figure_, global_step)
...
def train_net(split_df=None, base_path=Path("D:\TCGA_Data\Temp"), back_model=None):
    ...
    top = nn.Sequential(nn.Linear(num_ftrs, 2))
    model.fc = top
    ...
    data_loaders, dfs, class_weights, class_ratio, sarc_only_df = get_data_loaders(
        split_df, base_path
    )
    if optim == "adam":
    ...
    cycle_scheduler = OneCycleLR(
        optimizer,
        max_lr=lr,
        steps_per_epoch=len(data_loaders["Tr_eval"]),
        epochs=epochs,
        pct_start=0.3,
        div_factor=20,
    ...
    tr_metrics = {
        ...
        "agg_auc": agg.av_aggregation_metric(
            data_loaders["Train"].dataset.sizes, agg_topN, agg.auc_roc_compute_fn
        ),
        "agg_ap": agg.av_aggregation_metric(
            data_loaders["Train"].dataset.sizes, agg_topN, agg.ap_prc_compute_fn
        ),
    }
    val_metrics = {
        "accuracy": Accuracy(),
        ...
        "prc": PrecisionRecallCurve(activated_output_transform),
        "agg_auc": agg.av_aggregation_metric(
            data_loaders["Val"].dataset.sizes, agg_topN, agg.auc_roc_compute_fn
        ),
        "agg_ap": agg.av_aggregation_metric(
            data_loaders["Val"].dataset.sizes, agg_topN, agg.ap_prc_compute_fn
        ),
    }
    test_metrics = {
        "accuracy": Accuracy(),
        ...
        "prc": PrecisionRecallCurve(activated_output_transform),
        "agg_auc": agg.av_aggregation_metric(
            data_loaders["Test"].dataset.sizes, agg_topN, agg.auc_roc_compute_fn
        ),
        "agg_ap": agg.av_aggregation_metric(
            data_loaders["Test"].dataset.sizes, agg_topN, agg.ap_prc_compute_fn
        ),
    }
    evaluator_tr = create_supervised_evaluator(
        model, metrics=tr_metrics, device=device
    )  # , non_blocking=True)
    ...
    def log_training_loss(trainer):
        print(
            f"Epoch[{trainer.state.epoch}] Loss: {trainer.state.output:.2f} alpha: {alpha.alpha:.3f}"
        )
    ...
    def log_training_results(trainer):
        ...
        print(
            f"Training Results - Epoch: {trainer.state.epoch}  Avg accuracy: {metrics['accuracy']:.2f} Avg loss: {metrics['cel']:.2f} ap: {metrics['ap']:.3f} auc: {metrics['auc']:.3f} agg_ap: {metrics['agg_ap']:.3f} agg_auc: {metrics['agg_auc']:.3f}"
        )
        ...
        log_roc_image(
            preds, labels, metrics["auc"], tb_logger, "tile/roc_tr", trainer.state.epoch
        )
    ...
    def log_validation_results(trainer):
        ...
        print(
            f"Validation Results - Epoch: {trainer.state.epoch}  Avg accuracy: {metrics['accuracy']:.2f} Avg loss: {metrics['cel']:.2f} ap: {metrics['ap']:.3f} auc: {metrics['auc']:.3f} agg_ap: {metrics['agg_ap']:.3f} agg_auc: {metrics['agg_auc']:.3f}"
        )
    ...
    def update_datasets(trainer):
        ...
        for key in data_loaders.keys():
            ...
            if key == "Tr_eval":
                ...
                df_to_hdf5(
                    dfs[key], hdf_path, 1200, use_all=True, epoch=trainer.state.epoch
                )
            ...
            if key == "Train":
                if MIL_type == "sample" and phase == 1:
                    ...
                    h_weights = data_loaders[key].dataset.get_weights(
                        preds, alpha=samp_alpha
                    )
                else:
                    h_weights = data_loaders[key].dataset.get_weights(alpha=samp_alpha)
                if pre_stage == "sarc" and phase == 0:
                    ...
                    h_weights[labs == 1] = argh * h_weights[labs == 1]
                data_loaders[key].sampler.weights = torch.as_tensor(
                    h_weights, dtype=torch.double
                )
            elif key == "Val" and MIL_type == "sample" and phase == 1:
                v_preds, _ = val_metrics["agg_ap"].get_data()  # val metrics??
                v_preds = F.softmax(v_preds, 1)[:, 1]
    ...
    handler2 = Checkpoint(
        to_save,
        DiskSaver(base_path.joinpath("models"), create_dir=True),
        n_saved=2,
        filename_prefix="best",
        score_function=score_function,
        score_name="val_ap_auc",
        global_step_transform=global_step_from_engine(trainer),
    )
    ...
    def log_testing_results(trainer):
        if trainer.state.epoch > 5:
            ...
            np.savez(
                base_path.joinpath("w_hist.npz"),
                wh=np.vstack(data_loaders["Train"].dataset.weight_hist),
                sizes=data_loaders["Train"].dataset.get_sizes(),
                rh=np.vstack(data_loaders["Train"].dataset.raw_hist),
                labs=data_loaders["Train"].dataset.get_labels(),
            )
            np.savez(
                base_path.joinpath("w_hist_val.npz"),
                wh=np.vstack(data_loaders["Val"].dataset.weight_hist),
                sizes=data_loaders["Val"].dataset.get_sizes(),
                rh=np.vstack(data_loaders["Val"].dataset.raw_hist),
            ...
            checkpoint_fp = base_path.joinpath(
                "models",
                f"best_checkpoint_{tracker.best_epoch}_val_ap_auc={tracker.best_score:.4f}.pt",
            )
            ...
            result_str.append(
                f"Test Results - Epoch: {trainer.state.epoch}  Avg accuracy: {metrics['accuracy']:.2f} Avg loss: {metrics['cel']:.2f} ap: {metrics['ap']:.3f} auc: {metrics['auc']:.3f} agg_ap: {metrics['agg_ap']:.3f} agg_auc: {metrics['agg_auc']:.3f}"
            )
            print(result_str)
    ...
    if pre_stage == "none":
        ...
        if pre_stage == "sarc":
            ...
            df_to_hdf5(
                dfs["Train"], hdf_path, 1200, use_all=True, epoch=trainer.state.epoch
            )
            ds = hdf5Dataset(
                hdf_path,
                data_loaders["Train"].dataset.transform,
                data_loaders["Train"].dataset.target_transform,
            )
            h_weights = ds.get_weights(alpha=samp_alpha)
            if MIL_type != "batch":
                sampler = WeightedRandomStratifiedSampler(
                    h_weights,
                    int(np.max(sample_factor, sample_factor * len(ds))),
                    sizes=ds.sizes,
                    same_per_slide=same_per_slide,
                )
            else:
            ...
            h_weights = ds.get_weights(alpha=samp_alpha)
            sampler = WeightedMILSampler(
                h_weights, 70, sizes=ds.get_sizes(), btch_size=batch_size
            )
            data_loaders["Train"] = DataLoader(
                ds,
                batch_size=batch_size,
    ...
    tb_logger.close()
    # save info string
    with open(base_path.joinpath("info.txt"), "w") as f:
        f.write(info_str + ", ".join(result_str))
